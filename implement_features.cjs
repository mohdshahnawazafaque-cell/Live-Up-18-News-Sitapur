...
// 1. Update BreakingNews.tsx
const tickerCode = `import { useEffect, useState } from "react";
...
export default function BreakingNews() {
  ...
  return (
    <div className="bg-slate-900 border-b border-red-700 py-1 flex items-center h-12 shadow-inner">
      <div className="bg-red-600 text-white font-black text-sm uppercase px-4 h-full flex items-center shadow-lg whitespace-nowrap z-10">
        <span className="w-2 h-2 bg-white rounded-full animate-pulse mr-2"></span>
        {language === 'hi' ? 'ब्रेकिंग न्यूज़' : 'BREAKING NEWS'}
      </div>
      <div className="overflow-hidden whitespace-nowrap flex-1 h-full flex items-center relative text-white">
        <div className="animate-[marquee_25s_linear_infinite] hover:pause flex gap-12 pl-12">
          {breakingNews.map((news) => (
            <Link key={news.id} to={\`/article/\${news.id}\`} className="font-semibold text-sm hover:text-red-400 transition-colors flex items-center gap-2">
              <span className="text-red-500">•</span>
              {getLocalizedText(news, 'headline', language)}
            </Link>
          ))}
          {/* Default notice at the end */}
          <span className="font-bold text-yellow-400 flex items-center gap-2">
            <span className="text-red-500">•</span>
            विज्ञापन एवं समाचार के लिए संपर्क करें: 9956078419
          </span>
        </div>
      </div>
      <style>{`
...
`;
fs.writeFileSync('src/components/BreakingNews.tsx', tickerCode);
...
if (!article.includes('TeamMember')) {
  ...
  const fetchLogic = `
        ...
        const rel: NewsArticle[] = [];
        relatedSnap.forEach(d => {
          if (d.id !== fetchedArticle.id) {
            rel.push({ id: d.id, ...d.data() } as NewsArticle);
          }
        });
...
console.log("Completed adding features");
