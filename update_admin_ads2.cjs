...
const regex = /<\/div>\s*<\/div>\s*<div className="lg:col-span-2 space-y-6">/;
const newAdBlock = `
            </div>
          </div>
...
if (content.match(regex)) {
  ...
  console.log("Updated Admin with Ads using exact match");
} else {
  // Let's try finding the end of the sidebar
  const fallbackRegex = /<\/form>\s*<\/div>\s*<\/div>\s*<\/div>\s*<div className="lg:col-span-2">/;
  if (content.match(fallbackRegex)) {
    content = content.replace(fallbackRegex, newAdBlock.replace('lg:col-span-2 space-y-6', 'lg:col-span-2'));
    fs.writeFileSync('src/pages/Admin.tsx', content);
    console.log("Updated Admin with Ads using fallback match");
  } else {
